        ...
        foreach ($result as $item) {
            try {
                ...
                if (empty($endTime)) {
                    ...
                    $deliveryOrderSql = "SELECT * FROM delivery_order where create_time > '2023-10-01' AND finish_time >= '" . $startTime . "' AND finish_time <= '" . $endTime . "' and logistic_id = '" . $item['logistic_id'] . "'  AND status in (" . DeliveryOrder::STATUS_REJECT_IN_WAREHOUSE . "," . DeliveryOrder::STATUS_DELIVERED . "," . DeliveryOrder::STATUS_REPLACE_DELIVERED . ") AND is_logistic_company_settle = 0 ";
                if (!empty($logisticNo)) {
