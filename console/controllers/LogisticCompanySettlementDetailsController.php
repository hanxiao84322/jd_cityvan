...
//                                    echo "logisticCompanyFeeRulesSql:" . $logisticCompanyFeeRulesSql . "\r\n";
     ...
     * @param $endTime
     */
    public function actionJdRun($startTime = '', $endTime = '', $logisticId = '', $logisticNo = '')
    {
        $sql = "SELECT DISTINCT logistic_id FROM " . LogisticCompanyFeeRules::tableName() . " WHERE  type = " . LogisticCompanyFeeRules::TYPE_WAREHOUSE . "  ";
        if (!empty($logisticId)) {
            $sql .= " AND logistic_id =  " . $logisticId . "  ";
        }
        echo "sql:" . $sql . "\r\n";
            ...
            try {
                ...
                foreach ($deliveryOrderResult as $deliveryOrder) {
                    try {
                        ...
                        $logisticCompanyFeeRulesSql = "SELECT * FROM " . LogisticCompanyFeeRules::tableName() . " where type = " . LogisticCompanyFeeRules::TYPE_WAREHOUSE . " AND logistic_id = '" . $itemLogisticId . "' AND province = '" . $province . "' AND city = '" . $city . "' AND district = '" . $district . "'";
//                        echo "logisticCompanyFeeRulesSql:" . $logisticCompanyFeeRulesSql . "\r\n";
                        $logisticCompanyFeeRulesResult = \Yii::$app->db->createCommand($logisticCompanyFeeRulesSql)->queryOne();
                        if (empty($logisticCompanyFeeRulesResult)) {
                            $logisticCompanyFeeRulesSql = "SELECT * FROM " . LogisticCompanyFeeRules::tableName() . "  where type = " . LogisticCompanyFeeRules::TYPE_WAREHOUSE . " AND logistic_id = '" . $itemLogisticId . "' AND province = '" . $province . "' AND city = '" . $city . "'";
//                            echo "logisticCompanyFeeRulesSql:" . $logisticCompanyFeeRulesSql . "\r\n";
                            $logisticCompanyFeeRulesResult = \Yii::$app->db->createCommand($logisticCompanyFeeRulesSql)->queryOne();
                            if (empty($logisticCompanyFeeRulesResult)) {
                                if (empty($logisticCompanyFeeRulesResult)) {
                                    $logisticCompanyFeeRulesSql = "SELECT * FROM " . LogisticCompanyFeeRules::tableName() . "  where type = " . LogisticCompanyFeeRules::TYPE_WAREHOUSE . " AND logistic_id = '" . $itemLogisticId . "' AND province = '" . $province . "'";
//                                    echo "logisticCompanyFeeRulesSql:" . $logisticCompanyFeeRulesSql . "\r\n";
                                    $logisticCompanyFeeRulesResult = \Yii::$app->db->createCommand($logisticCompanyFeeRulesSql)->queryOne();
                                    if (empty($logisticCompanyFeeRulesResult)) {
                                        throw new \Exception("快递公司ID：" . $itemLogisticId . ",省：" . $province . ",市：" . $city . ",区/县：" . $district . "快递单号：" . $itemLogisticNo . "没有对应的运费规则");
